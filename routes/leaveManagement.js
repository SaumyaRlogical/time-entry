...
router.post(
  "/createLeaveEntry",
  [
    body("leaveType", "Must be Valid enum value leave Type").isIn([
      "Full Day",
      "First half day",
      "Second half day",
    ]),
    body("leaveReason", "Must be Valid enum value for Leave Reason type").isIn([
      "Casual Leave",
      "Emergency Leave",
      "Sick Leave",
    ]),
...
router.put(
  "/updateStatus/:id",
  [
    body("status", "Must be valid enum value").isIn([
      "Pending",
      "Approved",
      "Rejected",
      "Cancelled",
...
router.get("/getByUserId/:userId", async (req, res) => {
  try {
    ...
    console.log(userId);
    const user = await LeaveManagementEntity.find({ userId: userId }).populate({
      path: "userId",
      select: ["email", "name"],
    });
    res.send({ user });
